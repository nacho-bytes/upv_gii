    ...
    public Arista[] kruskal() {       
        ...
        for (int i = 0; i < numVertices(); i++){
            ...
            for (l.inicio(); !l.esFin(); l.siguiente()) {
                ...
                aristasFactibles.insertar(nueva);
            }
        }
        
        System.out.println("Aristas factibles" + aristasFactibles);
        
        int i = 0;
        
        while (i<numVertices()-1 && !aristasFactibles.esVacia()){
            Arista arista = aristasFactibles.eliminarMin();
            int origen = arista.origen, destino = arista.destino;
            int ccOrigen = cc.find(arista.origen), ccDestino = cc.find(arista.destino);
