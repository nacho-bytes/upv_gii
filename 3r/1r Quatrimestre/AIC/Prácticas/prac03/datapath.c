...
static void genera_latch6(elem_6i6o_t* reg, int x1, int y1, int ancho, int alto, int i1_y, int i2_y, int i3_y, int i4_y, int i5_y, int i6_y, char* nombre, char* in1, char* in2, char* in3, char* in4, char* in5, char* in6, char* tcolor, char* color, char * relleno) {
    ...
    sprintf((*reg).dibujo, "<g>  <polyline points=\"%d,%d %d,%d %d,%d %d,%d %d,%d\"\n     style=\"fill:%s;stroke:%s;stroke-width:%f\"/>\n", x1, y1, x1, y1 + alto, x1 + ancho, y1 + alto, x1 + ancho, y1, x1, y1, relleno, color, trazo_grueso);
    sprintf((*reg).dibujo, "%s  <text x=\"%d\" y=\"%d\" \n    style=\"fill:%s;stroke:%s;font-family:Arial;font-size:14;text-anchor:middle\">\n    %s\n</text>\n", (*reg).dibujo, x1 + ancho / 2, y1 - 10, color, "none", nombre);
    sprintf((*reg).dibujo, "%s  <text x=\"%d\" y=\"%d\" \n    style=\"fill:%s;stroke:%s;font-family:Arial;font-size:14;text-anchor:middle\">\n    %s\n</text>\n", (*reg).dibujo, (*reg).in1.x + ancho / 2, (*reg).in1.y - 4, tcolor, "none", in1);
    sprintf((*reg).dibujo, "%s  <text x=\"%d\" y=\"%d\" \n    style=\"fill:%s;stroke:%s;font-family:Arial;font-size:14;text-anchor:middle\">\n    %s\n</text>\n", (*reg).dibujo, (*reg).in2.x + ancho / 2, (*reg).in2.y - 4, tcolor, "none", in2);
    sprintf((*reg).dibujo, "%s  <text x=\"%d\" y=\"%d\" \n    style=\"fill:%s;stroke:%s;font-family:Arial;font-size:14;text-anchor:middle\">\n    %s\n</text>\n", (*reg).dibujo, (*reg).in3.x + ancho / 2, (*reg).in3.y - 4, tcolor, "none", in3);
    sprintf((*reg).dibujo, "%s  <text x=\"%d\" y=\"%d\" \n    style=\"fill:%s;stroke:%s;font-family:Arial;font-size:14;text-anchor:middle\">\n    %s\n</text>\n", (*reg).dibujo, (*reg).in4.x + ancho / 2, (*reg).in4.y - 4, tcolor, "none", in4);
    sprintf((*reg).dibujo, "%s  <text x=\"%d\" y=\"%d\" \n    style=\"fill:%s;stroke:%s;font-family:Arial;font-size:14;text-anchor:middle\">\n    %s\n</text>\n", (*reg).dibujo, (*reg).in5.x + ancho / 2, (*reg).in5.y - 4, tcolor, "none", in5);
...
static void genera_regs(elem_4i2o_t* regs, int x1, int y1, int ancho, int alto, int in1, char* in1_col, int in2, char* in2_col, int in3, char* in3_col, int in4, char* in4_col, char* color, char* relleno) {
    int x2;
    int y2;
    ...
    sprintf((*regs).dibujo, "%s  <text x=\"%d\" y=\"%d\" \n    style=\"fill:%s;stroke:%s;font-family:Arial;font-size:14;text-anchor:start\">\n    %d\n</text>\n", (*regs).dibujo, (*regs).in4.x + 5, (*regs).in4.y, in4_col, "none", in4);
...
void dibuja_datapath() {
    ...
    fprintf(f_salida_html, "<svg style=width=\"100%%\" viewbox=\"0 0 1100 600\" xmlns=\"http://www.w3.org/2000/svg\" version=\"1.1\" shape-rendering=\"geometricPrecision\" >\n");
    ...
    genera_alu(&aluPC, aluPC_x, aluPC_y, aluPC_ancho, aluPC_alto, "+", "black", "lightgray");
    ...
    /* Etapa ID **/
    genera_regs(&regs, regs_x, regs_y, regs_ancho, regs_alto, IF_ID.IR.Rfuente1, col_ID, IF_ID.IR.Rfuente2, col_ID, MEM_WB.IR.Rdestino, col_WB, WBdata, col_WB, "black", "lightgray");
    ...
    if latencia_salto1(solucion_riesgos_control) {
        // Cmp Zero
        genera_alu(&zeroID, zeroid_x, zeroid_y, zero_ancho, zero_alto, "cmp", "black", "lightgray");
        // genera_latch(&zero_id, zeroid_x, zeroid_y, zero_ancho, zero_alto, "", "zero", "red", "black", "lightgray");
        // Calculador nuevo PC
        genera_alu(&aluID, aluid_x, aluid_y, aluPC_ancho, aluPC_alto, "+", "black", "lightgray");
    ...
    /* Etapa EX **/
    genera_alu(&aluEX, alu_x, alu_y, alu_ancho, alu_alto, "ALU", "black", "lightgray");
    ...
    if latencia_salto2(solucion_riesgos_control) {
        genera_alu(&zeroEX, ZeroEX_x, ZeroEX_y, zero_ancho, zero_alto, "cmp", "black", "lightgray");
        genera_latch6(&RegEX_M, EXM_x, EXM_y, reg_ancho, reg_alto, RegID_EX.out1.y, zeroEX.out1.y, aluEX.out1.y, aluEX.pos2.y + 5 * sep1y, RegID_EX.out6.y, 0, "EX_M", "", "", "ALUout", "B", "IR", "", "red", "gray", "lightgray");
        // ctrl
    ...
    if (solucion_riesgos_datos == cortocircuito) {
        ...
        if (WBaEXalu_s) {
            ...
            muxctrl.ctrl = 1;
            sprintf(muxctrl.color, "%s", col_M);
        } else if (inst_salto_PCrel(ID_EX.IR.codop)) {
            if (!latencia_salto1(solucion_riesgos_control)) {
                muxctrl.ctrl = 0;
                sprintf(muxctrl.color, "%s", col_EX);
            } else {
