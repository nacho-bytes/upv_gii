...
static void inicializa_mips(
    ...
    /* Memoria de datos */
    valor.int_d = 0;
    for (i = 0; i < TAM_MEMO_DATOS; i += sizeof (byte)) {
        escribe_mem_datos(i, valor, t_byte, NO);
    } /* endfor */
    ...
    for (i = 0; i < TAM_MEMO_INSTRUC; i += sizeof (dword)) {
        escribe_mem_instruc(i, instr);
    } /* endfor */
...
(
    ...
    char * uso =
            ...
            "\t--no-show-RM  no mostrar registros y memoria\n"
            "\t--no-show-LOG no mostrar LOG\n"
            "\t--no-show-C   no mostrar CCONSOLA\n"
    ...
    if (!LIBMIPS || LIBMIPS && argc > 0) {
        ...
        while (1) {
            static struct option long_options[] = {
                ...
                {"no-show-RM", no_argument, &SHOW_REG_MEM, 0},
    ...
    if (LIBMIPS) {
        sprintf(nombre_fich, "%s", "fichero.s");
