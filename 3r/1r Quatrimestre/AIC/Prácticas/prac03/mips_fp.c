...
void fase_LS(short i) {
	...
	/* copiar operandos */
        ...
        LSn[2].address = mux_FP_LSsup(ID_LS.Ra, EX_MEM.ALUout, WBdata) + ID_LS.Imm;
        LSn[2].IR = ID_LS.IR;
        LSn[2].iPC = ID_LS.iPC;
    ...
    if (i == FP_LS_LAT) {
        ...
        switch (LS[i].IR.codop) {
            case OP_FP_L_D:
                datos_mem = lee_mem_datos(LS[i].address, t_double, &dirty);
                LS_WBn.fpALUout.fp_d = datos_mem.fp_d;
                break;
            case OP_FP_L_S:
                datos_mem = lee_mem_datos(LS[i].address, t_float, &dirty);
            ...
            case OP_FP_L_PS:
                ...
                datos_mem = lee_mem_datos(LS[i].address + 4, t_float, &dirty);
                LS_WBn.fpALUout.fp_ps[Hi] = datos_mem.fp_s;
                break;
            case OP_FP_S_D:
                //datos_mem.fp_d = LS[i].Fb.fp_d;
                datos_mem.fp_d = datos_mem_store.fp_d;
                escribe_mem_datos(LS[i].address, datos_mem, t_double, SI);
            ...
            case OP_FP_S_S:
                ...
                escribe_mem_datos(LS[i].address, datos_mem, t_float, SI);
                // pasar_a_FP_LS_WB_NOP();
                // Stores no pasan por WB        
                estat.instrucciones++;
                estat.multiciclo++;
            ...
            case OP_FP_S_PS:
                ...
                escribe_mem_datos(LS[i].address + 4, datos_mem, t_float, SI);
                // pasar_a_FP_LS_WB_NOP();
                // Stores no pasan por WB                
                estat.instrucciones++;
                estat.multiciclo++;
        ...
        case OP_FP_L_PS:
            for (i = 0; i < TAM_REGISTROS; i++) {
                if (Rfp[i].busy == Ciclo + FP_LS_LAT) {
                    ...
                    IFstall = SI;
                    estructural = SI;
                    write_log_1("Estructural_WB IDstall, IFstall", IF_ID.iPC, i);
