        ...
        default: /* el resto de instrucciones */
            ...
            switch (solucion_riesgos_datos) {
                ...
                case cortocircuito:
                    ...
                    /* WBtoEX */
                    if (lee_Rfte1(ID_EX.IR) && escribe_Rdst(MEM_WB.IR) &&
                            ID_EX.IR.Rfuente1 == MEM_WB.IR.Rdestino) {
                        WBaEXalu_s = SI;
                        result = wb;
                    }

                    if (lee_Rfte2(ID_EX.IR) && escribe_Rdst(MEM_WB.IR) &&
                    ...
                    /* MEMtoEX */
                    if (lee_Rfte1(ID_EX.IR) && escribe_Rdst(EX_MEM.IR) &&
                            ID_EX.IR.Rfuente1 == EX_MEM.IR.Rdestino) {
                        MEMaEXalu_s = SI;
                        result = mem;
                    }

                    if (lee_Rfte2(ID_EX.IR) && escribe_Rdst(EX_MEM.IR) &&
...
void detectar_riesgos_datos(void) {
    ...
    switch (solucion_riesgos_datos) {
        case parada:
            ...
            // Cas Rfte1
            if (lee_Rfte1(IF_ID.IR) && escribe_Rdst(ID_EX.IR) &&
                    IF_ID.IR.Rfuente1 == ID_EX.IR.Rdestino) {
                IFstall=SI;
                IDstall=SI;
            }
            // Cas Rfte2p
            if (lee_Rfte2(IF_ID.IR) && escribe_Rdst(ID_EX.IR) &&
                    IF_ID.IR.Rfuente2 == ID_EX.IR.Rdestino) {
                IFstall=SI;
                IDstall=SI;
            }
            /* Riesgo entre MEM e ID */
            // Cas Rfte1
            if (lee_Rfte1(IF_ID.IR) && escribe_Rdst(EX_MEM.IR) &&
                    IF_ID.IR.Rfuente1 == EX_MEM.IR.Rdestino) {
                IFstall=SI;
                IDstall=SI;
            }
            // Cas Rfte2p
            if (lee_Rfte2(IF_ID.IR) && escribe_Rdst(EX_MEM.IR) &&
                    IF_ID.IR.Rfuente2 == EX_MEM.IR.Rdestino) {
                IFstall=SI;
                IDstall=SI;
            }
            break;
        ...
        case cortocircuito:
            ...
            if (es_load(ID_EX.IR) && escribe_Rdst(ID_EX.IR) &&  
                    lee_Rfte1(IF_ID.IR) &&
                    IF_ID.IR.Rfuente1 == ID_EX.IR.Rdestino) {
                IFstall=SI;
                IDstall=SI;
