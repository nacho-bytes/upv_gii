...
static void imprime_nav2(char* archivo) {
    ...
    fprintf(f_salida_html, "<a href=\"final.html\">FINAL</a>&nbsp;\n");
    if (Ciclo > 5)
        fprintf(f_salida_html, "<a href=\"%s%03ld.html\">[-5]</a>&nbsp;\n", archivo,
            Ciclo - 5);
    else
        fprintf(f_salida_html, "[-5]&nbsp;\n");
    if (Ciclo > 1)
        fprintf(f_salida_html, "<a href=\"%s%03ld.html\">[-1]</a>&nbsp;\n", archivo,
            Ciclo - 1);
    else
    ...
    fprintf(f_salida_html, "&nbsp;&nbsp;&nbsp;\n");
    fprintf(f_salida_html, "<a href=\"estado%03ld.html\">Estado</a>\n", Ciclo);

    fprintf(f_salida_html, "&nbsp;&nbsp;&nbsp;\n");
    fprintf(f_salida_html, "<a href=\"crono%03ld.html\">Crono</a>\n", Ciclo);
    ...
    fprintf(f_salida_html, "Programa: %s\n", nombre_fich);
    fprintf(f_salida_html, "&nbsp;&nbsp;&nbsp;\n");
    fprintf(f_salida_html, "<b>Ciclo: %ld</b>\n\n", Ciclo);
}
...
static void imprime_nav2_css(char* archivo) {
    ...
    if (Ciclo > 10)
        fprintf(f_salida_html, "<a class=\"w3-bar-item w3-button w3-hover-grey\" href=\"%s%03ld.html\">[-10]</a>\n", archivo,
            Ciclo - 15);
    else
        fprintf(f_salida_html, "<a class=\"w3-bar-item w3-button w3-hover-grey\" href=\"%s%03ld.html\">[-10]</a>\n", archivo,
            Ciclo);
    ...
    if (Ciclo > 5)
        fprintf(f_salida_html, "<a class=\"w3-bar-item w3-button w3-hover-grey\" href=\"%s%03ld.html\">[-5]</a>\n", archivo,
            Ciclo - 5);
    else
        fprintf(f_salida_html, "<a class=\"w3-bar-item w3-button w3-hover-grey\" href=\"%s%03ld.html\">[-5]</a>\n", archivo,
            Ciclo);
    if (Ciclo > 1)
        fprintf(f_salida_html, "<a class=\"w3-bar-item w3-button w3-hover-grey\" href=\"%s%03ld.html\">[-1]</a>\n", archivo,
            Ciclo - 1);
    else
        fprintf(f_salida_html, "<a class=\"w3-bar-item w3-button w3-hover-grey\" href=\"%s%03ld.html\">[-1]</a>\n", archivo,
            Ciclo);
...
static void imprime_Dmem() {
    ...
    fprintf(f_salida_html, "<!-- Mem Datos -->\n");
    fprintf(f_salida_html, "<table>\n");

    if (!hay_double) {
        /*** Contenido de la memoria de datos, interpretada como enteros ***/

        fprintf(f_salida_html, "<tr bgcolor=\"%s\">\n", BG_OPER);
        fprintf(f_salida_html, "<td align=\"center\" colspan=\"9\">Memoria de Datos</td>\n");
        fprintf(f_salida_html, "<tr bgcolor=\"%s\">\n", BG_DIR1);
        fprintf(f_salida_html, "<td>&nbspDirección&nbsp</td>\n");

        for (i = 0; i < 8; i++) {
            fprintf(f_salida_html, "<td width=%d bgcolor=\"%s\" align=\"center\">+%lu</td>\n", MEM_W, BG_DIR2, i * sizeof (dword));
        } /* endfor */
        fprintf(f_salida_html, "</tr>\n");
        ...
        for (i = SegDatos.inicio; i < SegDatos.final; i += sizeof (dword)) {
            if (dir_dword(i) % 8 == 0) {
                fprintf(f_salida_html, "<tr bgcolor=%s>\n", BG_VAL1);
                fprintf(f_salida_html, "<td bgcolor=\"%s\" align=right>%d</td>\n", BG_VAL2, i);
            } /* endif */

            sprintf(aux, "" F_DWORD "", MD.m_dword[dir_dword(i)]);
            if (inst_load_int(EX_MEM.IR.codop) && (EX_MEM.ALUout == i)) {
                fprintf(f_salida_html, "<td bgcolor=\"%s\" align=right>%s</td>\n", col_M, aux);
            } else if (inst_store_int(EX_MEM.IR.codop) && (EX_MEM.ALUout == i)) {
                fprintf(f_salida_html, "<td bgcolor=\"%s\" align=right>%s</td>\n", col_M, aux);
            } else {
                fprintf(f_salida_html, "<td align=right>%s</td>\n", aux);
            }
        ...
        /*** Contenido de la memoria de datos, interpretada como dobles ***/

        fprintf(f_salida_html, "<tr bgcolor=\"%s\">\n", BG_OPER);
        fprintf(f_salida_html, "<td align=\"center\" colspan=\"9\">Memoria de Datos</td>\n");
        fprintf(f_salida_html, "<tr bgcolor=\"%s\">\n", BG_DIR1);
        fprintf(f_salida_html, "<td>Dirección</td>\n");
        for (i = 0; i < 8; i++) {
            fprintf(f_salida_html, "<td bgcolor=\"%s\" align=\"center\">+%ld</td>\n", BG_DIR2, i * sizeof (double));
        } /* endif */
        fprintf(f_salida_html, " </tr>\n");
        ...
        for (i = SegDatos.inicio; i < SegDatos.final; i += sizeof (double)) {
            if (dir_double(i) % 8 == 0) {
                fprintf(f_salida_html, "<tr bgcolor=%s>\n", BG_VAL1);
                fprintf(f_salida_html, "  <td align=center bgcolor=\"%s\">%d</td>\n", BG_VAL2, i);
            } /* endif */

            sprintf(aux, "%6.1lf", MD.m_double[dir_double(i)]);
            if (inst_load_fp(LS[FP_LS_LAT].IR.codop) && (LS[FP_LS_LAT].address == i)) {
                fprintf(f_salida_html, "<td bgcolor=\"%s\" align=right>%s</td>\n", col_LS, aux);
            } else if (inst_store_fp(LS[FP_LS_LAT].IR.codop) && (LS[FP_LS_LAT].address == i)) {
                fprintf(f_salida_html, "<td bgcolor=\"%s\" align=right>%s</td>\n", col_LS, aux);
            } else {
                fprintf(f_salida_html, "<td align=right>%s</td>\n", aux);
            }

            if (dir_double(i + sizeof (double)) % 8 == 0) {
                fprintf(f_salida_html, " </tr>\n");
            } /* endif */
        } /* endif */
...
static void imprime_Dmem_col() {
    ...
    fprintf(f_salida_html, " <tr bgcolor=\"%s\">\n", BG_HEADER);
    fprintf(f_salida_html, "  <td align=\"center\" colspan=5>Memoria de Datos</td>\n");
    fprintf(f_salida_html, "</tr>\n");
    fprintf(f_salida_html, "<tr>\n");

    fprintf(f_salida_html, "  <td bgcolor=\"%s\" align=\"center\">Dirección</td>\n", BG_DIR1);
    for (i = 0; i < 4; i++) {
        fprintf(f_salida_html, "  <td bgcolor=\"%s\" width=\"%d\" align=\"center\">+%d</td>\n", BG_DIR2, MEM_W, i);
    } /* endif */
    fprintf(f_salida_html, " </tr>\n");
    ...
    for (i = SegDatos.inicio; i < SegDatos.final; i++) {
        ...
        if (i % sizeof (word) == 0) {
            ...
            if (inst_load_int(EX_MEM.IR.codop) && (EX_MEM.ALUout == i)) {
                ...
                fprintf(f_salida_html, "  <td align=\"right\" bgcolor=\"%s\">"
                        S_WRITE " %s&nbsp;&nbsp;</td>\n", col_M, aux);
            } else if (inst_load_fp(LS[FP_LS_LAT].IR.codop) && (LS[FP_LS_LAT].address == i)) {
                // LOAD
                fprintf(f_salida_html, "  <td align=\"right\" bgcolor=\"%s\">"
                        S_READ " %s&nbsp;&nbsp;</td>\n", col_LS, aux);
            } else
                if (inst_store_fp(LS[FP_LS_LAT].IR.codop) && (LS[FP_LS_LAT].address == i)) {
                // STORE
                fprintf(f_salida_html, "  <td align=\"right\" bgcolor=\"%s\">"
                        S_WRITE " %s&nbsp;&nbsp;</td>\n", col_LS, aux);
            } else {
...
static void imprime_Imem_col() {
    ...
    for (i = SegInstruc.inicio; i < SegInstruc.final; i += sizeof (word)) {
...
static void imprime_senyales() {
    ...
    muestra_senyal(MEMaLS1_s, "MEMaLS1");
    muestra_senyal(0, " ");
    muestra_senyal(0, " ");
    ...
    muestra_senyal(WBaLS1_s, "WBaLS1");
    ...
    muestra_senyal(FP_WBaLS1_i, "WBFaLS");
...
static void imprime_dit(int num_instrucciones, char* archivo) {
    ...
    for (x = 0; x < MAX(MAX_CICLOS_INIC, maxima_columna_visible); x++) {
        fprintf(f_salida_html, "  <td width=\"%d\"><a href=\"%s%03ld.html\">%ld</a></td>\n",
                FASE_CRONO_W, archivo, primer_ciclo + x, primer_ciclo + x);
        fprintf(f_salida_html, "  <!-- T-Ciclo-H %lu %lu -->\n", x + primer_ciclo, x + primer_ciclo);
    }
    fprintf(f_salida_html, "</tr>\n");
...
void imprime_estado() {
    ...
    fprintf(f_salida_html, "</body></html>\n");
