...
void fase_EX_2_MEMDATOS(unsigned int oper) {
    ...
    if (Op[oper].ciclo == Op[oper].Teval) {
        ...
        if (ES_ESTACION(s, BUFFER_CARGA)) {
            ...
            switch (RS[s].OP) {
                case OP_LD:
                    LB[s].resultado = lee_mem_datos(mmem, LB[s].direccion, t_dword);
                    break;
                case OP_FP_L_D:
                    LB[s].resultado = lee_mem_datos(mmem, LB[s].direccion, t_double);
                    break;
                case OP_FP_L_S:
                    LB[s].resultado = lee_mem_datos(mmem, LB[s].direccion, t_float);
                    break;
                case OP_FP_L_PS:
                    LB[s].resultado = lee_mem_datos(mmem, LB[s].direccion, t_float_ps);
                    break;
                case OP_LB:
                    LB[s].resultado = lee_mem_datos(mmem, LB[s].direccion, t_byte);
                    break;
                case OP_LBU:
                    LB[s].resultado = lee_mem_datos(mmem, LB[s].direccion, t_byte);
                    ...
                    break;
                case OP_LH:
                    LB[s].resultado = lee_mem_datos(mmem, LB[s].direccion, t_half);
                    break;
                case OP_LHU:
                    LB[s].resultado = lee_mem_datos(mmem, LB[s].direccion, t_half);
                    ...
                    break;
                case OP_LW:
                    LB[s].resultado = lee_mem_datos(mmem, LB[s].direccion, t_word);
                    break;
                case OP_LWU:
                    LB[s].resultado = lee_mem_datos(mmem, LB[s].direccion, t_word);
            ...
            switch (RS[s].OP) {
                case OP_SD:
                    escribe_mem_datos(mmem, SB[s].direccion, SB[s].V2, t_dword);
                    break;
                case OP_FP_S_S:
                    escribe_mem_datos(mmem, SB[s].direccion, SB[s].V2, t_float);
                    break;
                case OP_FP_S_D:
                    escribe_mem_datos(mmem, SB[s].direccion, SB[s].V2, t_double);
                    break;
                case OP_FP_S_PS:
                    escribe_mem_datos(mmem, SB[s].direccion, SB[s].V2, t_float_ps);
                    break;
                case OP_SW:
                    escribe_mem_datos(mmem, SB[s].direccion, SB[s].V2, t_word);
                    break;
                case OP_SH:
                    escribe_mem_datos(mmem, SB[s].direccion, SB[s].V2, t_half);
                    break;
                case OP_SB:
                    escribe_mem_datos(mmem, SB[s].direccion, SB[s].V2, t_byte);
                    break;
                default:
                    fprintf(stderr, "ERROR (%s:%d): Operacion no implementada\n",
                            __FILE__, __LINE__);
