...
void swap( Byte a1[],Byte a2[],int rw,int rh,int w ) {
  ...
  if ( a1 != a2 ) {
    ...
    #pragma omp parallel for private(d, x, aux)
    for ( y = 0 ; y < rh ; y++ ) {
      ...
      for ( x = 0 ; x < rw ; x++ ) {
        ...
        //if (x == 0 && y == 0) printf("Nombre de fils: %d\n", omp_get_num_threads());
      }
    }
  }
}
...
void process( int w,int h,Byte a[], int bw,int bh ) {
  ...
  for ( y = bh ; y < h ; y += bh ) {
    ...
    #pragma omp parallel private(d, i, iMax, iMin)
    {  
      int iMin = INT_MAX;
      ...
      if (y == bh) printf("Hilo %d: %d iteraciones con distancias entre %d y %d\n.", 
        omp_get_thread_num(), i, iMin, iMax);
    } 
