...
class MySoldier(BDISoldier):
    def add_custom_actions(self, actions):
        ...
        @actions.add_function(".mejor_bid", (tuple, tuple,))
        def _mejor_bid(medpos, mipos):
            idx = 0
            distance = -1
            length = len(medpos)
        ...
        @actions.add_function(".get_point_closer", (tuple, tuple,))
        def _get_point_closer(pcentral,punto):
            haciadentro = np.subtract(pcentral, punto)
            norm = np.linalg.norm(haciadentro)
            if norm != 0:
                haciadentro = haciadentro/norm
            puntoN = tuple(np.add(punto, haciadentro))
            return puntoN
        
        @actions.add_function(".get_turn", (tuple, tuple,))
        def _get_turn(myPos, otherPos):
            myVec = pos_to_vec(myPos); otherVec = pos_to_vec(otherPos);
            turnVec = [(myVec[0] - otherVec[0]) + myVec[0], (myVec[1] - otherVec[1]) + myVec[1]];
            return vec_to_pos(turnVec);
