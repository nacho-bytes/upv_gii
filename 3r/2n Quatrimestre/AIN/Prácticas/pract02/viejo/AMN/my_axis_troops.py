...
class MySoldier(BDISoldier):
    def add_custom_actions(self, actions):
        ...
        def _is_friend_in_shoot(mPos, ePos, fPos, fDist):
            ...
            if (alpha < math.pi / 6): return True;
            else: return False;
...
class MyMedic(BDIMedic):
    def add_custom_actions(self, actions):
        ...
        @actions.add_function(".is_not_friend_in_shoot", (float,int,float,int,))
        def _is_not_friend_in_shoot(eAngle, eDist, fAngle, fDist):
            return MySoldier._is_not_friend_in_shoot(eAngle, eDist, fAngle, fDist);
